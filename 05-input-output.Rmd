...
below:^[Note
that `textConnection()` is used here to demonstrate the contents of a `.csv` file. Usually a file name such as `"nationalities.csv"` would be used instead.]

<!-- __XXX__ I think I would skip the textConnection & read.csv here. Just -->
<!-- give the three lines showing a csv file -->

```{r}
csv_file = textConnection("Person, Nationality
                           Robin, English
                           Colin, Scottish")
nationalities = read.csv(csv_file)
```
...
In general, you should never "hand-write" a CSV file. Instead you should use `write.csv` or
equivilent. The Internet Engineering Task Force has the [CSV definition](https://www.ietf.org/rfc/rfc4180.txt)
