...
The first section introduce **rio**, a 'meta package' for efficiently reading and writing data in a range of file formats, using two simple functions. Next we explore in more detail efficient functions for reading in files stored in common *plain text* file formats from the **readr** and **data.table** packages. Binary formats, which can dramatically reduce file sizes and read/write times, are covered next.
