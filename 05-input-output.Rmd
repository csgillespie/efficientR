...
The first section explains how a 'meta package',  **rio**, can be used to efficiently read and write data in a range of file formats using two simple functions. Next we explore in more detail efficient functions for reading in files stored in common *plain text* file formats from the **readr** and **data.table** packages. Binary formats, which can dramatically reduce file sizes and read/write times, are covered next.

With the accelerating digital revolution and growth in open data, an increasing proportion of the world's data can be downloaded from the internet. This trend is set to continue, making the next section on downloading and importing data from the web of great importance. Benchmarks throughout the chapter demonstrate that choice of file format and packages for data I/O can have a huge impact on computational efficiency. 
...
- If possible, keep the names of local files downloaded from the internet unchanged. This will help you trace the provenance of the data in the future.
...
**rio** is is a 'A Swiss-Army Knife for Data I/O', providing easy-to-use and highly performant wrapper functions for importing a range of file formats. As stated in the package's [vignette](https://cran.r-project.org/web/packages/rio/vignettes/rio.html), **rio** aims to "simplify the process of importing data into R and exporting data from R." The vignette goes on to to explain how many of the functions for data I/O described in R's [Data Import/Export manual](https://cran.r-project.org/doc/manuals/r-release/R-data.html) are out of date and difficult to learn.

<!-- __XXX__ Out of date; in what way? -->
...
equivilent function. The Internet Engineering Task Force has the [CSV definition](https://www.ietf.org/rfc/rfc4180.txt)
that facilites sharing CSV files between tools and operating systems.
```
...
```{r 5-1, fig.cap="Benchmarks of **base**, **data.table** and **readr** functions for reading csv files. The facets ranging from $2$ to $200$ represent the number of columns in the csv file.", echo=FALSE, fig.height=4, fig.width=6,warning=FALSE, message=FALSE, out.width="90%", fig.align="center"}
...
<!-- __XXX__ I would drop the table below and just quote the numbers in the text as needed. -->
<!-- The numbers are in the figure anyway. -->
...
<!-- __XXX__ Should mention tibble in the above section -->
...
We know that binary formats are advantageous from space and read/write time perspectives. The benchmarks in this section, based on the replicating the `co2` dataset used in the previous section, are designed to help understand *how much* more efficient they are. First we look at file size savings. Figure \@ref(fig:sizes) shows that the *relative* space saving of native R formats increase with dataset size. It is striking to note that binary `.Rds` files can be more than 100 times more space efficient than plain text `.csv` files: a 30 MB `.csv` containing replicated data from the `.co2` dataset contains the same information as  0.1 MB `.Rds` file!

```{r, echo=FALSE, eval=FALSE}
files_co2 = list.files(path = "data", pattern = "co2.", full.names = TRUE)
files_co2 = files_co2[!grepl("RData", files_co2)] # remove RData
filesize_co2 = data.frame(
  Format = gsub(pattern = "data/co2.", replacement = "", files_co2),
  size = file.size(files_co2) / 1000000,
  size_csv = max(file.size(files_co2) / 1000000),
  rel_size = (file.size(files_co2) / 1000000) /
    min(file.size(files_co2) / 1000000),
  n_row = nrow(df),
  n_row_rel = nrow(df) / nrow(df_co2)
  )
```

```{r spacebench, echo=FALSE, eval=FALSE}
library("feather") # load feather package
...
for(i in seq(500, 2000, 500)){
  ...
  files_co2 = list.files(path = tempdir(), pattern = "co2.", full.names = TRUE)
  
  filesize_co2_tmp = data.frame(
  Format = gsub(pattern = paste0(tempdir(), "/co2."), replacement = "", files_co2),
  size = file.size(files_co2) / 1000000,
  size_csv = max(file.size(files_co2) / 1000000),
  rel_size = (file.size(files_co2) / 1000000) /
    min(file.size(files_co2) / 1000000),
  ...
  filesize_co2 = rbind(filesize_co2, filesize_co2_tmp)
...
```{r sizes, echo=FALSE, fig.cap="Absolute (MB) and relative (compared with the Rds file format) file sizes of the replicated `co2` dataset saved in various file formats. Different compression ratios can be expected for more complex datasets."}
...
library("ggplot2")
ggplot(filesize_co2, aes(size_csv, rel_size)) +
  geom_line(aes(color = Format)) +
  xlab("Size of csv file (MB)") +
  ylab("Size relative to Rds")
```

The results of this simple disk usage benchmark show that saving data in a compressed binary format can improve efficiency from from hard-disk and, if your data will be shared on-line, data download time and bandwidth usage perspectives. But how does each method compare from a computational efficiency perceptive?
