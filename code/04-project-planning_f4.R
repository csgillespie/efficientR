...
abline(h=seq(0, 80, by=20), lty=3, col="grey80")


lines(res$MB[res$exp=="base_sqrbrkt"], res$Time[res$exp=="base_sqrbrkt"], 
      col=get_col(1), lwd=2)
text(408, 72, "base", col=get_col(1), pos=2)
lines(res$MB[res$exp=="dplyr_filter"], res$Time[res$exp=="dplyr_filter"], 
      col=get_col(2), lwd=2)
text(408, 45, "dplyr", col=get_col(2), pos=2)
lines(res$MB[res$exp=="dt_key"], res$Time[res$exp=="dt_key"], 
      col=get_col(3), lwd=2)
text(408, 4, "DT:Key", col=get_col(3), pos=2)
lines(res$MB[res$exp=="dt_standard"], res$Time[res$exp=="dt_standard"], 
      col=get_col(4), lwd=2)
text(408, 16, "DT", col=get_col(4), pos=2)

axis(1, seq(0, 400, 400), tick=FALSE)
axis(2, seq(0, 80, by=20), tick=FALSE)
