...
Data manipulation is a large topic with major potential implications for efficiency and there is an entire book written on the subject [@Spector2008].

## Data processing {#dplyr}

After the data has been tidied it is much easier to process. As with many aspects of R programming there are many ways to process a dataset, some more efficient than others. The focus is on the recently developed dplyr package, which  has a number of advantages compared with base R and in the package `data.table`:
...
Renaming data columns is a common data manipulation task made easy with dplyr'r function `rename()`.^[Note
