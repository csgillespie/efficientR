...
```{r, eval=FALSE, echo=FALSE}
res = NULL
for(i in seq(10, 300, length.out = 10)){
  idata_big = do.call("rbind", replicate(i, idata, simplify = FALSE))
  idata_dt = data.table(idata_big)
  idata_key = idata_dt
  setkey(idata_key, Country)
  mb = microbenchmark(
    times = 2,
    base = idata_big[idata_big$Country == "Australia", ],
    dplyr = filter(idata_big, Country == "Australia"),
    data.table = idata_dt[list("Australia"), ]
    # data.table_key = idata_key[list("Australia"), ]
  )
  tab = tapply(mb$time / 1000, mb$expr, mean)
  res_tmp = data.frame(
    exp = names(tab),
...
Figure \@ref(fig:dtplot) illustrates the speed improvement benefits of data.table for different datasets sizes. As with the **readr** vs base R issue, the results show that the relative benefits of **data.table** improve with dataset size.
