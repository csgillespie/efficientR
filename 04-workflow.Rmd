...
Note that **dplyr** does not rely on the `$` symbol: it knows that
that `Country` is a variable of `idata`
Because `idata` was the first argument, **dplyr** assumes
that any subsequent are variables.^[Note that this syntax is a defining feature of **dplyr**
...
Large datasets often contain much worthless or blank information, clogging up RAM and reducing computational efficiency.
...
# Method 2: using names assignment
