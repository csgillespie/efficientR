...
Most well documented packages provide some example data for you to play with. This can be really useful for demonstrating use cases in domain specific areas, that uses a particular data format. To see all the datasets associated with a particular package, use the function `data(package = "package_name")`. Datasets provided by **dplyr**, for example, can be viewed with `data(package = "dplyr", verbose = T)`.
