...
The usefulness of many datasets can be greatly enhanced if they are combined with other data. If we could merge the `co2_ems` data with data on geographic location, for example, we could see where the largest polluting countries are. 
...
Comparing the `co2_ems` data with this new dataset of the `world`, it is clear that the column `region` contains the same information in both. We can verify the extent of this overlap using the `%in%` functions, which finds out how many elements in one vector are in another vector:

```{r}
# save the result as 'm', for match
summary({m = co2_ems$Country %in% world$region})
```

This comparison exercise has been fruitful: almost 90% of the values in the former dataset can be found in the latter. But what about the country names that do now match? We can identify these as follows:

```{r}
unique(co2_ems$Country[!m])
```

It is clear from the output that some of the non-matches (e.g. the European Union) are not countries at all. However, others, such as 'Gambia, The' and the United States clearly should have matches in the world dataset. We can use fuzzy matching to rectify this:



llustrated below using the `left_join` function below.
