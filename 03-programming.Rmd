...
```{r 6-2, fig.cap="Example of Monte-Carlo integration. To estimate the area under the curve throw random points at the graph and count the number of points that lie under the curve.", echo=FALSE,fig.width=6, fig.height=4, fig.align="center", out.width="70%"}
...
<!-- __XXX__ Colin add a section on the apply family somewhere around here (reckon it deserves a section of its own) --> 
## The apply family

The apply functions can be an alternative to writing for loops. The general idea is to apply (or map) a function to 
each element of an object. For example, you can apply a function to row or column of a matrix, or each element of a list. 
A list of available functions is given in \@ref(tab:apply-family), with a short description. In
general, the apply functions have similar properties:
...
```{block, type="rmdnote"}
Most people rarely use the other apply functions. For example, I have only found `eapply` 
useful once. Students in my class uploaded R scipts. Using `source`, I was able to read
in the scripts to a separate environment. I then applied a marking scheme to each
environment using `eapply`. Using separate environments, avoided name classes.
```
...
The `apply` function is used to apply a function to the rows or columns of a matrix. In many data science
problems, this is a common task. For example,
to calculate the standard deviation of the row
```{r, results="hide"}
data("ex_mat", package="efficient")
# 1 is the maring and corresponds to rows
row_sd = apply(ex_mat, 1, sd)
```
The second argument is the `MARGIN`. This is a vector giving the subscripts which the function (the third argument)
...
```
Additional arguments can be passed to the function that is to be applied to the data, e.g.
```{r}
...
a list. Essentially, we apply a function to each element of a list or vector. Similarly, `sapply` and `vapply`, but the return type is not necessary 
a list.
...
The `ggplot2movies` package contains about sixty thousand movies. Each movies 


```{r}
data(movies, package="ggplot2movies")
ratings = movies[,7:16]
means = apply(ratings, 2, mean)
rating = 1:10
plot(rating, means, type="l")
sds = apply(ratings, 2, sd)
sds
segments(rating, means - 2*sds,  y1 = means + 2*sds)
...
most helpful.
  ...
  * The apply functions are an example of functional programming. Chapter 16 of _R for data Science_ describes 
  the interplay between loops and functional programming in more detail [@grolemund_r_2016], while @Wickham2014 
  gives a more in-depth description of the topic.
...
```{r 6-4, echo=FALSE, fig.height=4, fig.width=6, fig.cap="Comparsion of mean functions.", eval=TRUE, fig.align="center", out.width="70%"}
