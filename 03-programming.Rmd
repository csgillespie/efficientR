...
generate random numbers using `rnorm` the first argument is the `n`, if the number of
...
good
```
However, `bad` is a character string with class `try-error` and an `condition`
...
Discussed in more detail in [Hadley](http://adv-r.had.co.nz/Exceptions-Debugging.html)  - don't want to 
repeat. 

### Warnings: `warning` {-}

Warnings are generated using `warning`. When a warning is raised, it indicates
...
power laws, I use messages to give the user and estimate of expected run time. Similar
...
used in actual `print`/`show` methods, e.g. look at the function definition of the
...
error. For example, suppose we wish to download some kind of data from a web site. The
key idea is what can the user do if the function cannot complete it's purpose. 


The function `GET` from the **httr** package retrieves whatever information is
available. If the page exists, the function will return text (with the correct error
code), e.g.
```{r eval=FALSE}
...
Instead of raising an error we could instead return `NA` (the web-page isn't available).
However, for the typically use case, it's not clear how returning `NA` would be helpful.
...
### Example: Returning a missing value

  * `benchmarkme`: `get_cpu()`, `get_ram` returns `NA`
  * 
