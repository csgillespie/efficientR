...
Errors can be caught using `try` and `tryCatch` (discussed in more detail in section
XXX).
...
The function `GET` from the **httr** package retrieves whaterver information is
...
The `install_github` in the **devtools* package either installs the
...
In contrast, if `download.file` function fails, it raises a warning. The difference
between `download.file` and `install_github` is the expectation of failure. We don't 
expect `install_github` to fail, however, `download.file, 
