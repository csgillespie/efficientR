...
### Communicating with the user: messages vs warnings vs stop

When we create a function, we often want the function to give feedback on the current state. For example, are missing arguments or has
a numerical calculation failed. There are three main techniques of communicating with the user.

  * Fatal errors are raised by `stop`, i.e. execution is terminated. When `stop` is called, there is no way for a function to continue. For instance, if one of our arguments was the standard deviation, we could include the check
  ...
  Errors can be caught using `try` and `tryCatch` (discussed in more detail in section XXX).
  
  * Warnings are generated using `warning`. When a warning is raised, it indicates potential problems. For example, `mean(NULL)` returns `NA` and also raises a warning. Warnings can be hidden using `suppressWarnings`:
    ```{r}
  ...
  * To give informative output, use the `message` function. In my package for fitting power laws, I use messages to give the 
  user and estimate of expected run time. Similar to warnings, messages can be suppressed with `suppressMessages`.
  
  * Another function used for printing messages is `cat`. In general, `cat` should only be used in actual `print`/`show` methods.
