...
the we attempt to download data, the function also obtains an HTML status code. If the
page exists, the function will return text (with the correct status code), e.g.
```{r eval=FALSE}
GET("http://google.com/") # Status: 200
```
If the url is incorrect, e.g. a broken link or is a redirect, then we can interogate the
