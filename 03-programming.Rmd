...
can often mask deep issues that crept into our code longer term.
...
powerlaws, I use messages to give the user an estimate of expected run time. By
providing an rough estimate of how long the function takes, allows the user to
...
standard deviation of each column, we will use the standard deviation of all the 
entire data set
...
deviation is for early column is unnecessary. Instead, we should evaluate once and
store the result
```{r, results="hide"}
sd_x = sd(x)
...
If we we compare the two methods on a $100$ row by $1000$ column matrix, the cached 
