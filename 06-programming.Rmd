...
```{block, type="rmdnote"}
Everything in R is a function call. When calculate `1 + 1`, we are actually exercuting
`+(1, 1)`.
```
...
local(source("code/06-programming_f1.R", local=TRUE))
```
...
invisible. However, when we assign the function to an object, i.e. `out = regression_plot(x, y)`
...
If we sort `m` in the usual way `sort(m)`, we perform standard alpha-numeric ordering;
placing `December` first. This is technically correct, but not that helpful. We can
use factors to remedy this problem by specifying the admissible levels
...
`aov` is used to fit standard analysis of variance models. Potential bugs arise when a
variable is numeric, but in reality is a categorical variable.
...
In this section we will just discuss S3 since that is the most popular. The S3 system
...
the function `summary.data.frame` does not exist, then it `summary.default` is called
(if it exists).
...
res = rbenchmark::benchmark(m_plot_mpg(10), plot_mpg(10), columns = c("test", "relative", "elapsed"))
...
local(source("code/06-programming_f2.R", local=TRUE))
```
...
This chapter provides a brief foray into the word of parallel computing. The idea is
to give a flavour of what is possible, instead of covering all possible varities. For
a fuller account, see \@mccallum2011.
...
The computer used to compile the published version of this book chapter has `r no_of_cores` 
CPUs/Cores. 
...
`cl` object. However, if the parallel code is within function, it's possible that
