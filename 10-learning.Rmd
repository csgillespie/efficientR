...
The best place to look for help on R functions is often within R itself. Although R's internal documentation is notoriously terse, studying it as the first 'port of call' when stuck is time well spent: you will what the function was designed to do and perhaps gain further insight about its history, e.g. through references to academic papers.

All functions from proper functions have help files, which contain, at a minimum, a list of the input arguments and the nature of the output that can be expected. Prefixing a function name with `?` will show the help for that function. To the help file for `plot`, for example just
type:
