...
R's notoriety for being able to solve a problem in multiple different ways has only grown with the proliferation of community contributed packages. In this book we want to focus on the *best* way of solving problems, from an efficiency perspective. Often it is instructive to discover *why* a certain way of doing things is faster than others. However, if your aim is simply to *get stuff done*, you only need to know what is likely to be the most efficient way. In this way R's flexibility can be inefficient: although it is likely easier to find *a* way of solving any given problem in R than other languages, but it makes it harder to find *the best* way to solve that problem. This book tackles this issue head-on by recommend what we believe are the most efficient approaches. We hope you trust our views, based on years of R using and teaching, but we also hope that you question them at times and test them, with benchmarks, if you suspect there's a better way of doing things (thanks the R's flexibility and ability to interface with other languages there may well be).
