...
```{r, message=FALSE}
library("dplyr")
wb_ineq = rename(wb_ineq, Country = `Country Name`)
```
...
wb_ineq[id] = data.matrix(wb_ineq[id])
vapply(wb_ineq, class, character(1))
```
...
```{r, warning=FALSE}
wb_ineq[id] = as.numeric(unlist(wb_ineq[id]))
```
...
wb_ineq = mutate_each(wb_ineq, "as.numeric", id)
...
wb_ineq[id] = apply(wb_ineq[id], 2, as.numeric)
...
Note that **dplyr** does not use the `$` symbol. It knows that that `Country` is a variable of `wb_ineq`:
...
```{r}
library("data.table")
wb_ineq = readRDS("extdata/wb_ineq-renamed.Rds")
