...
Random access memory (RAM) is a type of computer memory that can be accessed randomly: any byte of memory can be accessed without touching the preceding bytes. RAM is found in computers, phones, tablets and even printers. The amount of RAM R has access to is increbibly important, as it impacts the speed of your analysis and the size of
your analysis.
 
```{r 3-1, fig.cap="Two $8$GB RAM cards. Credit: https://commons.wikimedia.org/wiki/File:Two_8_GB_DDR4-2133_ECC_1.2_V_RDIMMs.jpg", echo=FALSE}
...
Unless you have a fairly expensive laptop your computer probably has a standard hard disk drive (HDD). HDDs were first introduced by IBM in 1956. Data is stored using magnetism on a rotating platter, as shown in Figure \@ref(fig:3-2). The faster the platter spins, the faster the HDD can perform. Many laptop drives spin at either $5400$RPM (Revolutions per Minute) or $7200$RPM. The major advantage of HDDs is that they are cheap, making a $1$TB laptop standard.

```{r 3-2, fig.cap='A standard 2.5" hard drive, found in most laptops. Credit: https://en.wikipedia.org/wiki/Hard_disk_drive', echo=FALSE}
...
```{r 3-4, echo=FALSE, fig.width=8, fig.height=4, fig.cap="CPU benchmarks from the R package, **benchmarkme**. Each point represents an individual CPU result."}
library("benchmarkme")
plot_past()
```
