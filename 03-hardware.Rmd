...
```{r 3-4, echo=FALSE, fig.width=4, fig.height=4,  fig.cap="CPU benchmarks from the R package, **benchmarkme**. Each point represents an individual CPU result.", fig.keep="last"}
library("benchmarkme")
results = plot_past()
times = sort(tapply(results$time, results$id, mean))
times = times/min(times)
dd_tmp = data.frame(times, rank=1:length(times))
ggplot(dd_tmp, aes(rank, times)) + 
  geom_point(alpha=0.5, size=1.5) + 
  scale_y_log10(limit=c(1, 100)) + 
  theme_bw() + 
