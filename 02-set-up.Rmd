...
```{r 2-1, echo=FALSE, fig.cap="Output from a system monitor (`gnome-system-monitor` running on Ubuntu) showing the resources consumed by running the code presented in the second of the Exercises at the end of this section. The first increases RAM use, the second is single-threaded and the third is multi-threaded.", out.width="\\textwidth"}
...
This applies to an even greater extent to packages, which occasionally change substantially from one release to the next.
...
```{r rstudio, fig.cap="RStudio Panels", echo=FALSE, out.width="\\textwidth"}
...
4. Use the **Environment pane** to click on the data object `pop_change`. Note that this runs the command `View(pop_change)`, which launches an interactive data explore pane in the top left panel (see Figure \@ref(fig:2-3)).
    ...
    ```{r 2-3, echo=FALSE, fig.cap="The data viewing tab in RStudio.", out.width="\\textwidth"}
...
```{r blas-bench, fig.width=6, fig.height=4,echo=FALSE, out.width="0.7\\textwidth", fig.align="center", fig.cap="Performance gains obtained changing the underlying BLAS library (tests from `benchmark\\_std()`)."}
