...
Windows.^[By
'supports' we mean that all CRAN packages are automatically tested on these
systems. R also supports Solaris. R has been reported to run on more exotic
operating systems, including those used in smart phones and game consoles [@peng_data_2015].]
R is platform-independent, working equally well on each of these platforms.
There are some operating system-specific quirks, however. The default notation for file paths
in Windows, `\\`, does not work in R, for example (see Section \@ref(location)).

Basic system information can be queried from within R using `Sys.info()`:
