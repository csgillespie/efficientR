...
R version, R startup files, use of RStudio and
linear algebra library (BLAS) can make your R work faster.
The next chapter deals with the hardware set-up.
By the end of this chapter you will understand how to optimize your system to maximise the efficiency of your R programming.
The topics we will cover include:

- R and operating systems: Windows, Mac or Linux? What's the 'system state'?
- R version: how to keep your base R installation and packages up to date;
- R startup: how and why to adjust your `.Rprofile` and `.Renviron` files;
...
R is mostly platform independent, working equally well on Windows, Linux and Mac.
...
R is equally efficent across different operating systems.
Likewise, the same optimisation techniques will work equally well on different OSs.
Beyond the $32$ vs $64$ bit issue (covered in the next chapter), the main issue for many will be user friendliness and compatibility other programs used alongside R for work.
Changing operating system can be a time consuming and *inefficient* process.
Unless switching OSs for educational reasons, that could boost productivity in the longer term, our advice to stick to whatever OS you are most comfortable with.
...
Loading all packages at the begginning of a script is good practice as it ensures all dependencies have been installed *before* time is spent executing code.
...
Next (after the exercises) we take a look at how to configure R using startup files. 
...
It is worth spending time identifying which of these could be useful in your work and practicing interacting with RStudio rapidly with minimal reliance on the mouse.
