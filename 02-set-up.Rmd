...
2. Start an activity monitor then type and execute the following code.
In it `lapply()` (or parallel version `parLapply()`) is used to *apply* a function, `median()` over every element in a list (see Section \@ref(the-apply-family)). How do the system output logs results on your system compare to those presented in Figure \@ref(fig:2-1)? 
    ...
    r2 = parallel::parLapply(X, median) # runs in serial on Windows
    ```
...
Every time R starts a couple of files are initiated by default, as documented in `?Startup`.
This section explains how to customise these files, allowing you to save API key or load frequently used functions.
...
Two script files run each time R starts (unless one of the command line options outlined above is used):
