...
process. This can be started in modern Windows versions by typing `Ctl-Alt-Del` or by clicking the task bar and 'Start Task Manager'.
...
in abbreviated form. This is illustrated below (although with IDE-driven autocompletion, outlined in the next section, shorter functions are not always faster to enter.)
...
The Mac equivalent is usually found by simply replacing `Ctl` and `Alt` with the Mac-specific `Cmd` button.
...
`spdf@data$varname`.^['Slots' are sub-elements of an object analogous to a column in a `data.frame` but referred to with `@` not `$`.]
...
Creating R packages is easier than ever before, with tools such as **devtools** for managing R's quirks and making the process user friendly.
