...
Monitoring CPU load in this context can be useful for identifying whether R is running in parallel (see Figure \@ref{2-1}).
...
distributions. `htop` and Gnome's **System Monitor** (`gnome-system-monitor`, see Figure 2-1) are more refined alternatives which use command-line and graphical user interfaces respectively.
...
To add the RStudio mirror for Ubuntu 14.04 (codenamed Trusty), for example, add the following line to `/etc/apt/sources.list`:

```
echo 'deb https://cran.rstudio.com/bin/linux/ubuntu trusty`
```

In the above code `cran.rstudio.com` is the mirror and `trusty` is the Ubuntu version.
Then `r-base` and other `r-` packages can be installed.
...
Through its wide range of features RStudio can help make you a more efficient and productive R programmer, for example by reducing the amount of time spent remembering and typing function names thanks to intelligent autocompletion.
