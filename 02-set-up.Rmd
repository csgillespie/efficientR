...
some API keys, my `.Renviron` has a few other lines:
  ...
  * `R_DEFAULT_PACKAGES=utils,grDevices,graphics,stats,methods`. Explicitly state the 
    ...
    `methods` is always loaded. Due to historical reasons, under some circumstances
    the `methods` package isn't loaded by default.
...
```{r blas-bench, echo=FALSE, fig.cap="Example performance gains of installing OpenBLAS using tests from `benchmark_std()`."}
library("ggplot2")
res_blas = readRDS("data/res_rl_blas.Rds")
res_noblas = readRDS("data/res_rl_noblas.Rds")
res_blas$BLAS = "OpenBLAS"
res_noblas$BLAS = "No BLAS"
res_all = rbind(res_blas, res_noblas)
res_all$`Test group` = factor(res_all$test_group)
levels(res_all$`Test group`) = c("Programming", "Matrix calculations", "Matrix functions")
                              
ggplot(res_all) + geom_violin(aes(`Test group`, elapsed, fill = BLAS)) +
...
1. What BLAS system is your version of R using? (Hint: you will need the **benchmarkme** package installed.)
