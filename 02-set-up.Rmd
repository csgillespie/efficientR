...
This chapter is primarily about base R so for the most part all you need is R installed on your computer(see Section \@ref(install-rstudio)). We also discuss how your IDE set-up can boost efficiency and recommend RStudio. The only package used in **benchmarkme**.
...
2. Start an activity monitor then type and execute the following code. How do the results on your system compare to those presented in Figure \@ref(fig:2-1)? 
    ...
    ```{r, eval=FALSE}
    # Note: this will consume more than 2 GB RAM and may take several seconds to run depending on hardware
