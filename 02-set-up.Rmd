...
This applies to an even greater extent to packages, the behaviour of which can change substantially from one release to the next.
Depending on context, you may want to your R installation and packages up-to-date, or 'pin' the version of R and packages to specific versions, to ensure stability.
For most desktop users, keeping packages up-to-date is desirable: new code tends to be more efficient, intuitive, robust and feature rich. This section explains how.
...
The default for this function is for the `ask` argument to be set to `TRUE`, giving control over what is downloaded onto your system.^[In
the previous section we specified only a few packages to update.]
This is a good thing:
updating dozens of large packages can consume a large proportion of available system resources and much time!
