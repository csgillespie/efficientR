...
For this reason, giving a clear and consistent name to your objects, especially if they are going to be used many times in your script, can boost project efficiency (if an object is only used once, its name is less important, a case where `x` could be acceptable). Following discussion in [@] and variable names should be lower case, with an underscore `(_)` separating words. Unless you are creating an S3 object, avoid using a `.` in the name. Create names that are concise, but still have meaning. 
