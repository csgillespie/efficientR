...
with vectors from the **bit** package which take up just over a 16th of the space
...
Monopoly is the `move_square()` function. Within this function, we spend around 50%
...
and `lapply()`, so the barrier to using (this form) of parallel computing is low,
...
If you are using Linux or OS X, then another way of running code in parallel is to use 
