...
When developing code the speed bottleneck is fluid and moves. This means that
...
The chapter is organised as follows. First we begin with general hints and tips about optimising base R code.
...
Typically the `radix` (the non-default option) is optimal for most situations (it is around 
...
increasing order, `sort(x, decreasing=TRUE)` is marginally faster (around 10%) than `rev(sort(x))`.
