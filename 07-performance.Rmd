...
[Donald Knuth](https://en.wikiquote.org/wiki/Donald_Knuth) is a famous American computer
scientist is developed a number of key alogorithms that we used today. On the subject of optimisation, 
he said
...
speed boost. Some of the topics can provide increase the performance by orders of
magnitude, **but** require learning a new computer language (C++).
...
```{r cranlog, echo=FALSE, fig.cap="Downloads per day from the RStudio CRAN mirror of packages that provide R interfaces to other languages.", warning=FALSE}
local(source("code/10-performance_f2.R", local=TRUE))
```
...
In this simple example, the C++ variant is around $100$ times faster than the
corresponding R version.
