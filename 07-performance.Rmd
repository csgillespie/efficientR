...
computer scientist who developed a number of the key algorithms that we use today.
...
For this reason this chapter is covered towards the latter half of the book. The previous chapters deliberately focussed on concepts, packages and functions
...
If the length of `test` is equal to $1$, i.e. `length(test) == 1`, 
then the standard `if(test) yes else no` is more efficient.

<!-- __XXX__Colin__ Suggest a worked example is needed here -->
...
increasing order, then use the more efficient `sort(x, decreasing=TRUE)` instead
of `rev(sort(x))`.
...
faster. In some cases it's obvious where the bottle neck lies. Sometimes you will will instead guess, relying intuition. A drawback of this is that you could
...
Impressively, this refactored code runs 25 times faster than the original code.

<!-- __Colin__XXX Can I steal this example for the refactoring section? -->
